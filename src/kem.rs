...
impl KemKeyPair {
    ...
    pub fn encapsulate(pub_key: &KemPublicKey) -> Result<(Vec<u8>, SharedSecretKey)> {
        match pub_key.level {
            SecurityLevel::Level1 => {
                ...
                Ok((
                    ct.as_bytes().to_vec(),
                    SharedSecretKey::new(ss.as_bytes().to_vec()),
                ))
            }
            SecurityLevel::Level3 => {
        ...
        match self.inner.level {
            SecurityLevel::Level1 => {
                ...
                let ct = kyber512::Ciphertext::from_bytes(ciphertext).map_err(|_| {
                    PqcError::InvalidCiphertext("Invalid Kyber512 ciphertext".into())
                })?;
                Ok(SharedSecretKey::new(
            ...
            SecurityLevel::Level3 => {
                ...
                let ct = kyber768::Ciphertext::from_bytes(ciphertext).map_err(|_| {
                    PqcError::InvalidCiphertext("Invalid Kyber768 ciphertext".into())
                })?;
                Ok(SharedSecretKey::new(
            ...
            SecurityLevel::Level5 => {
                ...
                let ct = kyber1024::Ciphertext::from_bytes(ciphertext).map_err(|_| {
                    PqcError::InvalidCiphertext("Invalid Kyber1024 ciphertext".into())
                })?;
                Ok(SharedSecretKey::new(
    ...
    pub fn key_sizes(&self) -> KemSizes {
        match self.inner.level {
            SecurityLevel::Level1 => KemSizes {
                public_key: 800,
                private_key: 1632,
                ciphertext: 768,
                shared_secret: 32,
            },
            SecurityLevel::Level3 => KemSizes {
                public_key: 1184,
                private_key: 2400,
                ciphertext: 1088,
                shared_secret: 32,
            },
            SecurityLevel::Level5 => KemSizes {
                public_key: 1568,
                private_key: 3168,
                ciphertext: 1568,
                shared_secret: 32,
...
mod size_tests {
    ...
    fn fips203_kyber512_output_sizes() {
        ...
        assert_eq!(
            pk.as_bytes().len(),
            800,
            "Kyber512 public key: 800 bytes (FIPS 203 §7.1)"
        );
        let (ct, ss) = KemKeyPair::encapsulate(&pk).unwrap();
        assert_eq!(
            ct.len(),
            768,
            "Kyber512 ciphertext: 768 bytes (FIPS 203 §7.2)"
        );
        assert_eq!(
            ss.as_bytes().len(),
            32,
            "Kyber512 shared secret: 32 bytes (FIPS 203 §7.2)"
        );
    }
    ...
    fn fips203_kyber768_output_sizes() {
        ...
        assert_eq!(
            pk.as_bytes().len(),
            1184,
            "Kyber768 public key: 1184 bytes (FIPS 203 §7.1)"
        );
        let (ct, ss) = KemKeyPair::encapsulate(&pk).unwrap();
        assert_eq!(
            ct.len(),
            1088,
            "Kyber768 ciphertext: 1088 bytes (FIPS 203 §7.2)"
        );
        assert_eq!(
            ss.as_bytes().len(),
            32,
            "Kyber768 shared secret: 32 bytes (FIPS 203 §7.2)"
        );
    }
    ...
    fn fips203_kyber1024_output_sizes() {
        ...
        assert_eq!(
            pk.as_bytes().len(),
            1568,
            "Kyber1024 public key: 1568 bytes (FIPS 203 §7.1)"
        );
        let (ct, ss) = KemKeyPair::encapsulate(&pk).unwrap();
        assert_eq!(
            ct.len(),
            1568,
            "Kyber1024 ciphertext: 1568 bytes (FIPS 203 §7.2)"
        );
        assert_eq!(
            ss.as_bytes().len(),
            32,
            "Kyber1024 shared secret: 32 bytes (FIPS 203 §7.2)"
        );
    }
    ...
    fn shared_secret_not_all_zeros() {
        ...
        assert!(
            recovered.as_bytes().iter().any(|&b| b != 0),
            "Recovered secret must not be all zeros"
        );
    }
}
...
mod tests {
    ...
    fn test_wrong_key_gives_different_secret() {
        ...
        assert_ne!(
            alice_s, eve_s,
            "Security failure: different keys produced same secret"
        );
    }

...
impl DsaKeyPair {
    ...
    pub fn public_key(&self) -> DsaPublicKey {
        DsaPublicKey {
            ...
            bytes: self.inner.pk_bytes.clone(),
        }
    }
    ...
    pub fn verify_with_typed_key(
        ...
        match pub_key.level {
            SecurityLevel::Level1 => {
                ...
                let sig = dilithium2::DetachedSignature::from_bytes(signature).map_err(|_| {
                    PqcError::InvalidSignature("Invalid Dilithium2 signature".into())
                })?;
            ...
            SecurityLevel::Level3 => {
                ...
                let sig = dilithium3::DetachedSignature::from_bytes(signature).map_err(|_| {
                    PqcError::InvalidSignature("Invalid Dilithium3 signature".into())
                })?;
            ...
            SecurityLevel::Level5 => {
                ...
                let sig = dilithium5::DetachedSignature::from_bytes(signature).map_err(|_| {
                    PqcError::InvalidSignature("Invalid Dilithium5 signature".into())
                })?;
    ...
    pub fn key_sizes(&self) -> DsaSizes {
        match self.inner.level {
            SecurityLevel::Level1 => DsaSizes {
                public_key: 1312,
                private_key: 2528,
                signature: 2420,
            },
            SecurityLevel::Level3 => DsaSizes {
                public_key: 1952,
                private_key: 4000,
                signature: 3293,
            },
            SecurityLevel::Level5 => DsaSizes {
                public_key: 2592,
                private_key: 4864,
                signature: 4595,
            },
...
mod size_tests {
    ...
    fn fips204_dilithium2_output_sizes() {
        ...
        assert_eq!(
            pk.as_bytes().len(),
            1312,
            "Dilithium2 public key: 1312 bytes (FIPS 204 §7.1)"
        );
        assert_eq!(
            sig.len(),
            2420,
            "Dilithium2 signature: 2420 bytes (FIPS 204 §7.2)"
        );
    }
    ...
    fn fips204_dilithium3_output_sizes() {
        ...
        assert_eq!(
            pk.as_bytes().len(),
            1952,
            "Dilithium3 public key: 1952 bytes (FIPS 204 §7.1)"
        );
        assert_eq!(
            sig.len(),
            3293,
            "Dilithium3 signature: 3293 bytes (FIPS 204 §7.2)"
        );
    }
    ...
    fn fips204_dilithium5_output_sizes() {
        ...
        assert_eq!(
            pk.as_bytes().len(),
            2592,
            "Dilithium5 public key: 2592 bytes (FIPS 204 §7.1)"
        );
        assert_eq!(
            sig.len(),
            4595,
            "Dilithium5 signature: 4595 bytes (FIPS 204 §7.2)"
        );
    }
...
mod tests {
    ...
    fn test_tampered_message_rejected() {
        ...
        assert!(
            kp.verify(b"tampered message", &sig).is_err(),
    ...
    fn test_wrong_key_rejected() {
        ...
        assert!(
            kp2.verify(b"message", &sig).is_err(),
...
impl DsaKeyPair {
    ...
    pub fn from_bytes(
        ...
        match level {
            SecurityLevel::Level1 => {
                dilithium2::PublicKey::from_bytes(pk_bytes).map_err(|_| {
                    crate::error::PqcError::InvalidKey(
                        "Invalid Dilithium2 public key length".into(),
                    )
                })?;
                dilithium2::SecretKey::from_bytes(sk_bytes).map_err(|_| {
                    crate::error::PqcError::InvalidKey(
                        "Invalid Dilithium2 secret key length".into(),
                    )
                })?;
            }
            SecurityLevel::Level3 => {
                dilithium3::PublicKey::from_bytes(pk_bytes).map_err(|_| {
                    crate::error::PqcError::InvalidKey(
                        "Invalid Dilithium3 public key length".into(),
                    )
                })?;
                dilithium3::SecretKey::from_bytes(sk_bytes).map_err(|_| {
                    crate::error::PqcError::InvalidKey(
                        "Invalid Dilithium3 secret key length".into(),
                    )
                })?;
            }
            SecurityLevel::Level5 => {
                dilithium5::PublicKey::from_bytes(pk_bytes).map_err(|_| {
                    crate::error::PqcError::InvalidKey(
                        "Invalid Dilithium5 public key length".into(),
                    )
                })?;
                dilithium5::SecretKey::from_bytes(sk_bytes).map_err(|_| {
                    crate::error::PqcError::InvalidKey(
                        "Invalid Dilithium5 secret key length".into(),
                    )
                })?;
            }
        }

...
use pqc_vault::{dsa::DsaKeyPair, kem::KemKeyPair, SecurityLevel};
...
fn bench_kem_keygen(c: &mut Criterion) {
    ...
    for level in [
        ...
        g.bench_with_input(
            ...
            &level,
            |b, &l| b.iter(|| KemKeyPair::generate(black_box(l)).unwrap()),
        );
    }
    g.finish();
}
...
fn bench_kem_encapsulate(c: &mut Criterion) {
    ...
    for level in [
        ...
        g.bench_with_input(
            BenchmarkId::from_parameter(format!("{:?}", level)),
            &level,
            |b, _| b.iter(|| KemKeyPair::encapsulate(black_box(&pub_key)).unwrap()),
        );
    }
    g.finish();
}
...
fn bench_kem_decapsulate(c: &mut Criterion) {
    ...
    for level in [
        ...
        g.bench_with_input(
            BenchmarkId::from_parameter(format!("{:?}", level)),
            &level,
            |b, _| b.iter(|| kp.decapsulate(black_box(&ct)).unwrap()),
        );
    }
    g.finish();
}
...
fn bench_dsa_keygen(c: &mut Criterion) {
    ...
    for level in [
        ...
        g.bench_with_input(
            BenchmarkId::from_parameter(format!("{:?}", level)),
            &level,
            |b, &l| b.iter(|| DsaKeyPair::generate(black_box(l)).unwrap()),
        );
    }
    g.finish();
}
...
fn bench_dsa_sign(c: &mut Criterion) {
    ...
    for level in [
        ...
        g.bench_with_input(
            BenchmarkId::from_parameter(format!("{:?}", level)),
            &level,
            |b, _| b.iter(|| kp.sign(black_box(msg)).unwrap()),
        );
    }
    g.finish();
}
...
fn bench_dsa_verify(c: &mut Criterion) {
    ...
    for level in [
        ...
        g.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    DsaKeyPair::verify_with_typed_key(
                        black_box(&pub_key),
                        black_box(msg),
                        black_box(&sig),
                    )
